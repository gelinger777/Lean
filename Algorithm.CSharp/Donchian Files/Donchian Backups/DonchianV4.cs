...
namespace QuantConnect.Algorithm.CSharp
{
	...
	/// </summary>
	public class DonchianLiveAlgorithm : QCAlgorithm
	{
		// EDITABLE VARIABLES
		decimal risk = .2m;
		...
		private static bool pythonAllSymbols = false; //make true once confirmed the spy works on its own
		private static bool pythonLiveModeOnly = false; //make true once confirmed all works
		...
		private static bool pythonPredictions = true;
		private static bool reduceMaxPositions = true;
		bool closingHour = false;
		...
		public override void Initialize()
		{
			...
			Debug("Attempting to add " + SymbolInfo.Count() + " Symbols to the Algoithim");
			...
			Train(DateRules.EveryDay("SPY"), TimeRules.At(21, 0), () =>
			{
				if (pythonPredictions && (LiveMode || !pythonLiveModeOnly))
				{
					if (pythonAllSymbols)
					{
						foreach (Symbol sym in Securities.Keys)
						{
							python.UpdateMarketState(this, sym);
						}
					}
					else
					{
						python.UpdateMarketState(this, spy);
					}

					Debug("Current State of the market = " + python.GetSymbolState(spy));

					if (python.GetSymbolState(spy) == PythonMethods.MarketState.Bear)
					{
						maxLongDirection = 1;
						maxShortDirection = 6;
					}
					else if (python.GetSymbolState(spy) == PythonMethods.MarketState.Bull)
					{
						maxLongDirection = 6;
						maxShortDirection = 1;
					}
			...
			Train(DateRules.MonthStart("SPY"), TimeRules.At(19, 0), () =>
			{
				if (pythonPredictions && (LiveMode || !pythonLiveModeOnly))
				{
					if (pythonAllSymbols)
					{
						foreach (Symbol sym in Securities.Keys)
						{
							python.RefitModels(this, sym);
						}
					}
					else
						python.RefitModels(this, spy);
				}
			});
		}
		...
		public void OnData(Slice data)
		{
			...
			if (LiveMode || !pythonLiveModeOnly)
				reduceMaxPositions = false;
			if (IsWarmingUp)
			{
				PortValue = Portfolio.TotalPortfolioValue;
				return;
			...
			foreach (var index in symbols)
			{
				...
				try
				{
					if (data.ContainsKey(SymbolInfo[i].Symbol))
					{
						...
						if (i == "JNJ")
							Log("Processing next trade bar"); // Remove once proven this is working
						...
						if (reduceMaxPositions && pythonPredictions && portfolioOverloaded)
						{
							if (Portfolio[i].Invested && python.GetSymbolState(Portfolio[i].Symbol) != PythonMethods.MarketState.Unknown && python.GetSymbolState(Portfolio[i].Symbol) != python.GetSymbolState(Portfolio["SPY"].Symbol))
							{
								Log("Reduce Maximum Direction: closing losing and small winning positions");
			...
			foreach (var index in symbols)
			{
				...
				{
					Debug("Error Processing Portfolio Management - " + e.Message);
				}
			}
			Holdings = 0;
		}
