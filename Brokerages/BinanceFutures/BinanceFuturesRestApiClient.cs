...
namespace QuantConnect.Brokerages.BinanceFutures
{
    ...
    /// </summary>
    public class BinanceFuturesRestApiClient : IDisposable
    {
        private const string RestApiUrl = "https://fapi.binance.com";
        private readonly string userDataStreamEndpoint = "/fapi/v1/listenKey";
        ...
        /// </summary>
        public event EventHandler<BinanceFuturesOrderSubmitEventArgs> OrderSubmit;
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="BinanceFuturesRestApiClient"/> class.
        /// </summary>
        ...
        public BinanceFuturesRestApiClient(SymbolPropertiesDatabaseSymbolMapper symbolMapper, ISecurityProvider securityProvider, string apiKey, string apiSecret)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public IEnumerable<Messages.AccountPositions> GetAccountHoldings()
        {
            var timestampResult = CheckAutoTimestamp();

            var queryString = $"timestamp={GetNonce()}";
            var endpoint = $"/fapi/v1/positionRisk?{queryString}&signature={AuthenticationToken(queryString)}";
            ...
            return JsonConvert.DeserializeObject<Messages.AccountPositions[]>(response.Content);
        }
        ...
        public Messages.AccountInformation GetCashBalance()
        {
            ...
            var queryString = $"timestamp={GetNonce()}";
            var endpoint = $"/fapi/v1/account?{queryString}&signature={AuthenticationToken(queryString)}";
            var request = new RestRequest(endpoint, Method.GET);
            request.AddHeader(KeyHeader, ApiKey);
        ...
        public IEnumerable<Messages.OpenOrder> GetOpenOrders()
        {
            ...
            var queryString = $"timestamp={GetNonce()}";
            var endpoint = $"/fapi/v1/openOrders?{queryString}&signature={AuthenticationToken(queryString)}";
            var request = new RestRequest(endpoint, Method.GET);
            request.AddHeader(KeyHeader, ApiKey);
            ...
            if (response.StatusCode != HttpStatusCode.OK)
            {
                throw new Exception($"BinanceBrokerage.GetOpenOrders: request failed: [{(int)response.StatusCode}] {response.StatusDescription}, Content: {response.Content}, ErrorMessage: {response.ErrorMessage}");
            }
        ...
        public bool PlaceOrder(Orders.Order order)
        {
            ...
            IDictionary<string, object> body = new Dictionary<string, object>()
            {
                ...
                { "reduceOnly",  (order.Properties as BinanceFuturesOrderProperties)?.ReduceOnly.ToString(CultureInfo.InvariantCulture) }
            };
            ...
            switch (order.Type)
            {
                case OrderType.Limit:
                    body["type"] = (order.Properties as BinanceFuturesOrderProperties)?.PostOnly == true
                        ? "LIMIT_MAKER"
                        : "LIMIT";
        ...
        public bool CancelOrder(Orders.Order order)
        {
            ...
            foreach (var id in order.BrokerId)
            {
                ...
                var request = new RestRequest("/fapi/v1/order", Method.DELETE);
                request.AddHeader(KeyHeader, ApiKey);
                request.AddParameter(
                    "application/x-www-form-urlencoded",
                    Encoding.UTF8.GetBytes(body.ToQueryString()),
        ...
        public IEnumerable<Messages.Kline> GetHistory(Data.HistoryRequest request)
        {
            ...
            string endpoint = $"/fapi/v1/klines?symbol={symbol}&interval={resolution}&limit=1000";
        ...
        {
            var endpoint = $"/fapi/v1/depth?symbol={_symbolMapper.GetBrokerageSymbol(symbol)}&limit=1000";
            var request = new RestRequest(endpoint, Method.GET);
            var response = ExecuteRestRequest(request);
        ...
        public DateTime GetServerTime(bool resetAutoTimestamp = false)
        {
            ...
            {
                var response = ExecuteRestRequest(request);
                var result = JsonConvert.DeserializeObject<Messages.BinanceCheckTime>(response.Content);
            ...
            else
            {
                ...
                if (TotalRequestsMade == 1)
                {
                    ...
                    if (!response.IsSuccessful)
                        return DateTime.UtcNow + timestampOffset;
                    result = JsonConvert.DeserializeObject<Messages.BinanceCheckTime>(response.Content);
                    TotalRequestsMade++;
                }
        ...
        protected void OnOrderSubmit(Messages.NewOrder newOrder, Order order)
        {
            try
            {
                OrderSubmit?.Invoke(
                    this, 
                    new BinanceFuturesOrderSubmitEventArgs(newOrder.Id, order));
