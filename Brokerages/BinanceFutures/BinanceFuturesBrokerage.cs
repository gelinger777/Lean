...
namespace QuantConnect.Brokerages.BinanceFutures
{
    ...
    public partial class BinanceFuturesBrokerage : BaseWebsocketsBrokerage, IDataQueueHandler
    {
        private const string _wssUrl = "wss://fstream.binance.com/ws";
        ...
        private readonly BinanceFuturesRestApiClient _apiClient;
        ...
        public BinanceFuturesBrokerage(string apiKey, string apiSecret, IAlgorithm algorithm, IDataAggregator aggregator)
            : base(_wssUrl, new WebSocketClientWrapper(), null, apiKey, apiSecret, "BinanceFutures")
        {
            ...
            _apiClient = new BinanceFuturesRestApiClient(
                _symbolMapper,
                algorithm?.Portfolio,
                apiKey,
                apiSecret);
        ...
        {
            var positions = _apiClient.GetAccountHoldings();
            var holdings = positions?.Where(position => position.PositionAmt != 0);
        ...
        public override List<CashAmount> GetCashBalance()
        {
            ...
            var balances = account.Assets?.Where(balance => balance.MarginBalance != 0);
            if (balances == null || !balances.Any())
                return new List<CashAmount>();

            return balances
                .Select(b => new CashAmount(b.WalletBalance, b.Asset.LazyToUpper()))
                .ToList();
        }
                ...
                switch (item.Type.LazyToUpper())
                {
                    ...
                    default:
                        OnMessage(new BrokerageMessageEvent(BrokerageMessageType.Error, -1,
                            "BinanceFuturesBrokerage.GetOpenOrders: Unsupported order type returned from brokerage: " + item.Type));
                        continue;
                }

                order.Quantity = item.Quantity;
                order.BrokerId = new List<string> { item.Id };
                order.Symbol = _symbolMapper.GetLeanSymbol(item.Symbol, SecurityType.Equity, Market.BinanceFutures);
        ...
        private void OnOrderSubmit(BinanceFuturesOrderSubmitEventArgs e)
        {
